...
class JSONDataParser(Dataset):
    ...
    def encode_dataset(self, word2idx, label2idx, pos2idx, predicate2idx):
        ...
        for sentence, labels, sentence_pos, sentence_predicate in tqdm(
                zip(self.data_x, self.data_y, self.pos_x, self.predicates_x),
            ...
            predicates_x_stoi.append(
                torch.LongTensor([predicate2idx.get(predicate, 1) for predicate in sentence_predicate]).to(self.device))
...
class RoBERTaJSONDataParser(Dataset):
    def __init__(self, file_name, device):
        self.encoded_data = []
        self.data_x, self.data_y = [], []
        self.pos_x, self.predicates_x = [], []
    ...
    def get_sentences(self):
        for i in tqdm(range(len(self.sentences)), desc='Fetching sentences and labels', leave=False):
            ...
            for _, k in enumerate(roles_):
                ...
                predicates_sentence_ = ["_"] * len(predicates_sentence)
                predicates_sentence_[k] = verb_atlas_grp
                lemmas_sentence_.append('[SEP]')
                lemmas_sentence_.append(str(verb_atlas_grp))
                lemmas_sentence_.append('[SEP]')
    ...
    # def encode_dataset(self, word2idx, label2idx, pos2idx, predicate2idx):
    def encode_dataset(self, label2idx, pos2idx, predicate2idx):
        tokenizer = AutoTokenizer.from_pretrained(self.model_name)
        for sentence, labels, sentence_pos, sentence_predicate in tqdm(
                zip(self.data_x, self.data_y, self.pos_x, self.predicates_x),
                desc='Encoding Data', leave=False, total=len(self.data_x)):
            encoded_dict = tokenizer.encode_plus(text=sentence,
                                                 add_special_tokens=True,
                                                 return_attention_mask=False,
                                                 return_tensors='pt')
            self.encoded_data.append({'inputs': torch.squeeze(encoded_dict['input_ids']),
                                      # torch.LongTensor([word2idx.get(word, 1) for word in sentence]).to(self.device),
                                      'pos': torch.LongTensor([pos2idx.get(pos_tag, 1) for pos_tag in sentence_pos]),
                                      'predicates': torch.LongTensor(
                                          [predicate2idx.get(predicate, 1) for predicate in sentence_predicate]),
                                      'outputs': torch.LongTensor([label2idx.get(tag) for tag in labels])})
    ...
    def pad_batch(batch):
        ...
        return {'inputs': pad_sequence(inputs_batch, batch_first=True, padding_value=1),
                'pos': pad_sequence(pos_inputs_batch, batch_first=True),
                'predicates': pad_sequence(predicates_inputs_batch, batch_first=True),
                'outputs': pad_sequence(outputs_batch, batch_first=True)}
